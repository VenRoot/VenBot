...
export const Ban = async (e: Context, global: boolean) => 
{
    ...
    if(e.message.reply_to_message === undefined) return e.reply("Please reply to a message");
...
export const Kick = async (e: Context, global: boolean) =>
{
    ...
    let date = await ParseDate(e.message.text.split(" ")[1], chat.id).catch(i => {ReportError(i);}) as Date;
...
export const Mute = async (e: Context) =>
{
  ...
  let date = await ParseDate(e.message.text.split(" ")[1], e.chat.id).catch(i => {ReportError(i);}) as Date;
