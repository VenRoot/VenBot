...
export const changeList = {
    ...
    {
        let data = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "users.json"), "utf8")) as users[];
...
export const usersWhoNeedToAccept = {
    has: (userid: number) =>
    {
        let _tmp = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "uwnta.json"), "utf8")) as number[];
        if(_tmp.includes(userid)) return true;
        return false;
    },
    add: (userid: number) => 
    {
        let data = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "uwnta.json"), "utf8")) as number[];
        data.push(userid);
    ...
    getAll: () => JSON.parse(fs.readFileSync(path.join(__dirname, "..", "uwnta.json"), "utf8")) as number[]
};