...
docker build -t dockerreg.$SERVER:443/venbot:latest .
docker push dockerreg.$SERVER:443/venbot:latest

echo "Releasing to $SERVER"

scp docker-compose.yml $SERVER:~/Applications/VenBot/docker-compose.yml

ssh $SERVER "cd ~/Applications/VenBot; sudo -S docker compose pull; sudo -S docker compose up -d"
